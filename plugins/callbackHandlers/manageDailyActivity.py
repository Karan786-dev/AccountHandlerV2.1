...
@Client.on_callback_query(filters.regex(r'^/changeUnmuteProbability'))
async def changeUnmuteProbabilityQuery(bot: Client,query: types.CallbackQuery):
    channelID = query.data.split(maxsplit=1)[1]
    await query.message.delete()
    await query.message.reply("<b>Send new unmute probability</b>",reply_markup=cancelKeyboard)
    unmuteProbability: str = (await bot.listen(query.message.chat.id)).text
    if unmuteProbability == cancelButtonText: 
        text , keyboard = await viewChannelActivity(int(channelID))
        return await query.message.reply(text=text,reply_markup=keyboard)
    elif not unmuteProbability.isdigit():
        await query.message.reply("<b>Invalid input, please try again</b>")
        return await changeUnmuteProbabilityQuery(bot,query)
    
    await query.message.reply("<b>Unmute probability updated successfully</b>")
    ActivityChannels.update_one(
        {"channelID":int(channelID)},
        {"$set":{"unmuteProbability":int(unmuteProbability.replace("%",""))}}
    )
    text , keyboard = await viewChannelActivity(int(channelID))
    await query.message.reply(text=text,reply_markup=keyboard)
