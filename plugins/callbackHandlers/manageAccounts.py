...
async def assignAsSyncerConfirmHandler(_,query:CallbackQuery):
    ...
    subprocess.run(["pm2", "restart", SYNCBOTMAIN_PROCESS])
    text , keyboard = await account_details_view(updateData)
    await query.message.edit(text,reply_markup=keyboard)
    
@Client.on_callback_query(filters.regex(r'^/toggleHelperSync'))
async def helperBotToggle(_: Client,query: CallbackQuery):
    phone_number = query.data.split(maxsplit=1)[1]
    text = f"🚨 Are you sure to assign this account as Helper bot\nThis bot will help to get channels data\n\n<b>Make sure you login this account in your device</b>"
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("❌ No", f"/viewAccount {phone_number}"),
         InlineKeyboardButton("✅ Yes", f"/_confirmHelperBot {phone_number}")]
    ])
    await query.message.edit(text,reply_markup=keyboard)
    
@Client.on_callback_query(filters.regex(r'^/_confirmHelperBot'))
async def confirmHelperBot(_: Client,query: CallbackQuery):
    phone_number = query.data.split(maxsplit=1)[1]
    oldSyncBot = Accounts.find_one({"helperBot":True})
    if oldSyncBot and oldSyncBot.get("phone_number") == phone_number: return await query.answer("This account is already assigned as syncer")
    if oldSyncBot:
        oldSyncBotClient: Client = await UserbotManager.getSyncBotClient()
        if oldSyncBotClient:
            try: await oldSyncBotClient.disconnect()
            except: pass
        Accounts.update_one({"phone_number":oldSyncBot.get("phone_number")},{"$unset":{"helperBot":True}})
    updateData = Accounts.find_one_and_update({"phone_number":phone_number},{"$set":{"helperBot":True}},return_document=True)
    await query.message.edit("Assigning To Sync Bot...")
    try: await UserbotManager.getSyncBotClient(isNew=True)
    except Exception as error: return await query.message.reply(f"<b>[Error]</b>\n<pre>{error}</pre>")
    text , keyboard = await account_details_view(updateData)
    await query.message.edit(text,reply_markup=keyboard)
...
async def confirmAccountRemove(_, query):
    ...
    UserbotManager.stop_account(phone_number)
